...
## Setting Experiment
# It is very common to have multiple notebooks for one set of experiments.
# That can make managing the various experiments quite painful
# This way we can set the experiment first
# We use the username so that if multiple people run the code there won't be issues
username = spark.sql("SELECT current_user()").first()['current_user()']
experiment_path = f'/Users/{username}/advanced-mlflow-examples'
...
with mlflow.start_run(run_name='untuned_random_forest'):
  ...
  # You can also use mlflow.log_params which takes a dictionary in instead
  mlflow.log_param('n_estimators', n_estimators)
  # Use the area under the ROC curve as a metric. There is also log_metrics
  mlflow.log_metric('auc', auc_score)
  wrappedModel = SklearnModelWrapper(model)

  # Log the model with a signature that defines the schema of the model's inputs and outputs. 
  # When the model is deployed, this signature will be used to validate inputs.
  # Signature is also needed for use with databricks Model Serving
...
# MAGIC
# MAGIC With the integration of mlflow with UC, it is now possible to log models in with your data schemas
...
catalog_name = f'advanced_mlflow_example'
schema_name = 'ml'
model_name = "wine_quality"
uc_name = f'{catalog_name}.{schema_name}.{model_name}'

spark.sql(f"CREATE CATALOG IF NOT EXISTS {catalog_name}")
spark.sql(f"CREATE SCHEMA IF NOT EXISTS {catalog_name}.{schema_name}")

# To use uc we need to set the registry first
mlflow.set_registry_uri('databricks-uc')
model_version = mlflow.register_model(f"runs:/{run_id}/random_forest_model", uc_name)
...
# MAGIC %md You should now see the model in the Models page. We can tag it with tags 
# MAGIC
...
client = MlflowClient()
client.set_registered_model_alias(uc_name, "champion", model_version.version)

# COMMAND ----------

# MAGIC %md The Models page now shows the model tag "champion"
...
new_model_version = mlflow.register_model(f"runs:/{best_run.run_id}/model", uc_name)
